...
class DroneRacingDataGenerator(object):
    def __init__(self,
        ...
        self.client = airsimdroneracinglab.MultirotorClient()
    ...
    def load_level(self, level_name='Qualifier_Tier_3'):
        self.client.simLoadLevel(level_name)
        time.sleep(2)
        self.curr_track_gate_poses = []
        for gate_object in self.client.simListSceneObjects(".*[Gg]ate.*"):
            self.curr_track_gate_poses.append(self.client.simGetObjectPose(gate_object))

    def distort_track(self, Gatepositions):
        """"""
        cartesian_min = -25
        cartesian_max = 25
        new_x = Gatepositions.position.x_val + random.randrange(cartesian_min, cartesian_max)
        new_y = Gatepositions.position.y_val + random.randrange(cartesian_min, cartesian_max)
        new_z = Gatepositions.position.z_val + random.randrange(int(cartesian_min/10), int(cartesian_max/10))

        return [new_x, new_y, new_z, Gatepositions.orientation]
    ...
    def set_current_track_gate_poses_from_default_track_in_binary(self):
        ...
        self.curr_track_gate_poses = [self.client.simGetObjectPose(str(gate_name)) for gate_name in
        ...
        for idx in range(len(self.gate_object_names_sorted)):
            self.client.simSpawnObject(self.gate_object_names_sorted[idx], "CheckeredDroneGate16x16",
    ...
    def takeoff_with_moveOnSpline(self, takeoff_height, vel_max, acc_max):
        self.client.moveOnSplineAsync(path=[airsimdroneracinglab.Vector3r(4, -2, takeoff_height)],
    ...
    def expert_planner_controller_callback(self):
        ...
        if (self.curr_track_gate_poses is not None):
            dist_from_next_gate = math.sqrt(
                (self.curr_xyz[0] - self.curr_track_gate_poses[self.next_gate_idx].position.x_val) ** 2
    ...
    def fly_to_next_gate_with_moveOnSpline(self):
        ...
        if not self.perpendicular:
            ...
            gate_vector = racing_utils.geom_utils.get_gate_facing_vector_from_quaternion(
                self.curr_track_gate_poses[self.next_gate_idx].orientation, self.direction, scale=vel_max / 1.5)
            self.last_future = self.client.moveOnSplineVelConstraintsAsync(
                [self.curr_track_gate_poses[self.next_gate_idx].position],
                [gate_vector],
                vel_max=self.vel_max, acc_max=self.acc_max,
                add_curr_odom_position_constraint=True,
                add_curr_odom_velocity_constraint=True,
    ...
    def set_pose_of_gate_just_passed(self):
        if (self.last_gate_passed_idx == -1):
            return
        self.client.simSetObjectPose(self.gate_object_names_sorted[self.last_gate_passed_idx],
                                     self.next_track_gate_poses[self.last_gate_passed_idx])
    ...
    def set_pose_of_gate_passed_before_the_last_one(self):
        ...
        if (self.train_lap_idx > 0):
            if (self.last_gate_passed_idx in [-1, 0]):
                print("last_gate_passed_idx", self.last_gate_passed_idx, "moving gate idx from CURRENT track",
                      gate_idx_to_move)
                self.client.simSetObjectPose(self.gate_object_names_sorted[gate_idx_to_move],
                                             self.curr_track_gate_poses[gate_idx_to_move])
                return
            else:
                print("last_gate_passed_idx", self.last_gate_passed_idx, "moving gate idx from NEXT track",
                      gate_idx_to_move)
                self.client.simSetObjectPose(self.gate_object_names_sorted[gate_idx_to_move],
    ...
    def start_training_data_generator(self, num_training_laps=10, level_name='Qualifier_Tier_3'):
        self.load_level(level_name)
        # todo encapsulate in functions
        self.client.enableApiControl(True)
        time.sleep(0.01)
        self.client.disarm(vehicle_name='drone_0')
        time.sleep(0.01)
        self.client.setTrajectoryTrackerGains(airsimdroneracingvae.TrajectoryTrackerGains().to_list(),
                                              vehicle_name=self.drone_name)
        time.sleep(0.01)
...
if __name__ == "__main__":
    os.startfile("D:\GP\ADRL\ADRL.exe")
    time.sleep(5)
